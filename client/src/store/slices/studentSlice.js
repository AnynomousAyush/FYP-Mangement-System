...
export const fetchAllSupervisor = createAsyncThunk(
  ...
  async(_, thunkAPI) => {
    try {
      ...
      toast.error(error.response.data.message || "Failed to fetch available supervisor");
      return thunkAPI.rejectWithValue(error.response.data.message);
    }
  }
);
...
export const requestSupervisor = createAsyncThunk(
  ...
  async(data, thunkAPI) => {
    try {
      const res = await axiosInstance.post("/student/request-supervisors", data);
...
export const uploadFiles = createAsyncThunk(
  ...
  async({ projectId, files}, thunkAPI) => {
    try {
      ...
      toast.success(res.data.message || "Files uploaded successful")
